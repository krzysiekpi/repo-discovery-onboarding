...
Onboarding goal for Backend Developer: follow the focused track below, trace one relevant workflow, then make one small verified contribution.

## 2. First Day Plan

1. Read the root `README.md` and the generated discovery report for this repo.
2. Use the `Backend Developer` track in Section 8; do not try to learn every folder first.
...
Purpose: preserve API contracts, service boundaries, persistence behavior, and auth/error semantics.

API routes to trace first:
- GET `/admin/users` -> `list_users()` calls select, func.count, query.where, count_query.where, User.email.ilike
- GET `/admin/users/{user_id}` -> `get_user_detail()` calls _get_user_or_404, logger.info, _build_user_detail
- PATCH `/admin/users/{user_id}` -> `update_user()` calls _get_user_or_404, db.commit, db.refresh, logger.info, _build_user_detail
- PATCH `/admin/users/{user_id}/subscription` -> `update_subscription()` calls _get_user_or_404, SubscriptionStatus, db.commit, db.refresh, logger.info
- DELETE `/admin/users/{user_id}` -> `delete_user()` calls _get_user_or_404, db.delete, db.commit, logger.info
- POST `/admin/users/{user_id}/block` -> `block_user()` calls _get_user_or_404, db.commit, db.refresh, logger.info, _build_user_detail
- POST `/admin/users/{user_id}/unblock` -> `unblock_user()` calls _get_user_or_404, db.commit, db.refresh, logger.info, _build_user_detail
- GET `/admin/config` -> `get_config()` calls app_config_service.get_config, AdminConfigResponse, app_config_service.apply_config_to_runtime
- PATCH `/admin/config` -> `update_config()` calls app_config_service.get_or_create_config, logger.info, db.flush, app_config_service.apply_config_to_runtime, AdminConfigResponse
- GET `/admin/models` -> `get_available_models()` calls AdminAvailableModelsResponse
...
Avoid: duplicating business logic in route handlers when a service/model layer already owns it.
