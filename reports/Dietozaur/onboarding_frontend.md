...
Onboarding goal for Frontend Developer: follow the focused track below, trace one relevant workflow, then make one small verified contribution.

## 2. First Day Plan

1. Read the root `README.md` and the generated discovery report for this repo.
2. Use the `Frontend Developer` track in Section 8; do not try to learn every folder first.
...
Purpose: make the user-facing UI easier to inspect, navigate, and verify.

UI routes:
- `/login` -> `LoginPage` from `packages/admin/src/App.tsx:85`
- `/auth/verify` -> `LoginPage` from `packages/admin/src/App.tsx:86`
- `/users` -> `ProtectedRoute` from `packages/admin/src/App.tsx:88`
- `/users/:userId` -> `ProtectedRoute` from `packages/admin/src/App.tsx:98`
- `/config` -> `ProtectedRoute` from `packages/admin/src/App.tsx:108`
- `*` -> `Navigate` from `packages/admin/src/App.tsx:117`
- `/` -> `SplashPage` from `packages/webapp/src/App.tsx:103`
- `/onboarding` -> `OnboardingPage` from `packages/webapp/src/App.tsx:104`

Do first:
- Start at `packages/admin/src/App.tsx`, `packages/webapp/src/App.tsx`, then inspect the page/component tied to your route.
- Follow UI data loading through the frontend API/data-loading layer before changing state shape.
- Run frontend test/typecheck/build commands before handing off UI work.

Avoid: adding UI states that hide loading, error, auth, or persistence status.
